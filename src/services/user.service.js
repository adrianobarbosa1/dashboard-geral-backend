...
const createUserCpf = async (userBody) => {
  if (await User.isCpfTaken(userBody.username)) {
    throw new ApiError(httpStatus.BAD_REQUEST, 'CPF já existe');
  }
  // eslint-disable-next-line no-param-reassign
  userBody.password = await userBody.username;
  // eslint-disable-next-line no-param-reassign
  userBody.name = await userBody.name.toUpperCase();
  return User.create(userBody);
};
...
const updateAcessoTrue = async (username) => {
  const user = await getUserByCpf(username);
  if (!user) {
